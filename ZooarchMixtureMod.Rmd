...
```{r table 1: Definitions of key terms used in this paper, echo = FALSE}
...
```{r table_2: Measurements included in the simulation analyses, echo = FALSE}
...
```{r figure_3, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", out.width = "\\textwidth", fig.cap = "Posterior distributions of model hyper-parameters from sample of known-identity specimens and proposed prior distributions for mixture model applications"}
...
simulation_summary <- list(
  ...
  single_assemblage_n_individuals = single_assemblage_mixmod_data[, .N, `Accession  no`][, .N],
...
#add in footnotes manually (for in-pdf table)
...
```{r table_3: Group composition of the simulated measurement assemblages (element portions), echo = FALSE}
...
```{r making tables 4 and 5, include = FALSE, eval = TRUE}
...
```{r table_4: Elemental composition of the Pinarbaşı B sheep measurement assemblage, echo = FALSE}
#export table 4
write.csv(table_4, file = "./Output/Tables/Table 4 - Pinarbasi B Composition.csv", row.names = F)
#
kbl(table_4, caption = "Elemental composition of the Pinarbaşı B sheep measurement assemblage", booktabs = T, linesep = '') %>%
  kable_styling(latex_options = c("hold_position"))
```
...
```{r table_5: Elemental composition of the Northwest Anatolian cattle measurement assemblages, echo = FALSE}
#exporting table
write.csv(table_5, file = "./Output/Tables/Table 5 - Northwest Anatolian Composition.csv", row.names = F)

#
kbl(table_5, caption = "Elemental composition of the Northwest Anatolian cattle measurement assemblages", booktabs = T, linesep = '') %>%
...
#functions to simulate a specimen as immature/female/male and count up the composition
...
```{r figure_7, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", out.width = "\\textwidth", fig.cap = "Posterior distributions of model parameters for the simulated multisite assemblages with relevant population parameter values"}
...
Figure 9A shows the posterior distributions assemblage-level $\pi$ mixture components. In general, the Pinarbaşı B sheep assemblage is overwhelmingly composed of immature animals (posterior $\mu_{\pi_1}$ median = `r pinarbasi_posterior_summary$pinarbasi_pi1_median`; 95% posterior confidence interval for $\mu_{\pi_1}$ = `r pinarbasi_posterior_summary$pinarbasi_pi1_95CI`), somewhat lower than the observed fusion rate of proximal and middle phalanges `r pinarbasi_posterior_summary$pinarbasi_age_observation`. The posterior distribution of the overall adult sex ratio ($\theta_{\operatorname{female}}$) suggests that females are more common than males (`r pinarbasi_posterior_summary$pinarbasi_femaletheta_over50` of the posterior samples are above 0.5), but this estimate is uncertain, owing to the low overall proportions of female and male animals.
...
The archaeological case studies highlight the importance of considering the presence of immature specimens and elemental variation in body size when summarizing the biometry and composition of an assemblage, problems addressed by the Bayesian multilevel mixture model. Variation in the proportion of immature animals in the assemblage, as in the multisite case study for northwestern Anatolia, may also highlight differences in culling strategies or even the seasonality of animal presence at the sites. Most of the Pinarbaşı B material derives from immature specimens, which could complicate inter-assemblage biometric analyses that do not use sex-specific size estimates (e.g., Arbuckle et al., 2014). Restricting measurements only to fused specimens removes useful information, particularly when fusion rates may differ between male and female animals (Zeder & Hesse, 2000); further, it does not resolve the problem of immature animals in the measurement assemblage if early-fusing elements like the distal humerus are still included because of post-fusion growth (Popkin et al., 2012). The ability to create sex-specific biometric estimates is important to document large-scale spatial and temporal dynamics in animal body size (e.g., Arbuckle et al., 2016).

Examining LSIe (or LSI10) values of Pinarbaşı B sheep without accounting for the impact of immature animals would mislead a researcher into believing that sheep were smaller, on average, than sheep from contemporaneous sites (e.g., late Çatalhöyük: Baird et al., 2011). Sex-specific size estimates, accounting for the presence of immature animals, show that the Pinarbaşı B sheep are similar in size to contemporaneous Çatalhöyük sheep (Wolfhagen, Twiss, Mulville, & Demirergi, 2021). This bolsters the interpretation that Pinarbaşı B and Çatalhöyük sheep are the same animal population, and thus that the site was occupied seasonally by Çatalhöyük herders. While the estimated average LSIe for female sheep (µ2) at Pinarbaşı B is relatively broad (95% credible interval: -0.13 – -0.06 LSIe) because of the small proportion of adult-sized females in the assemblage (95% credible interval of the proportion of adult-sized females π2: 2–14%), it can still be fruitfully compared to other assemblages (95% credible interval of µ2 for late Çatalhöyük sheep: -0.11 – -0.06 LSIe; Wolfhagen et al., 2021). 
...
kbl(table_simulated_multisite_site_parameters[, .(Parameter, `Posterior Mean`, `Posterior Median`, `Posterior SD`, `$5\\%$ Quantile`, `$95\\%$ Quantile`, `$\\hat{R}$`, `Effective Sample Size (Bulk)`, `Effective Sample Size (Tail)`)],
    col.names = c("Parameter", "Posterior Mean", "Posterior Median", "Posterior SD", "$5\\%$ Quantile", "$95\\%$ Quantile", "$\\hat{R}$", "Effective Sample Size (Bulk)", "Effective Sample Size (Tail)"),
    caption = "Posterior fit summaries for site-specific model parameters (multisite simulation)",
...
kbl(table_nw_anatolian_model_parameters[, .(Parameter, `Posterior Mean`, `Posterior Median`, `Posterior SD`, `$5\\%$ Quantile`, `$95\\%$ Quantile`, `$\\hat{R}$`, `Effective Sample Size (Bulk)`, `Effective Sample Size (Tail)`)],
    col.names = c("Parameter", "Posterior Mean", "Posterior Median", "Posterior SD", "$5\\%$ Quantile", "$95\\%$ Quantile", "$\\hat{R}$", "Effective Sample Size (Bulk)", "Effective Sample Size (Tail)"),
    caption = "Posterior fit summaries for model parameters (Northwestern Anatolian cattle)",
    booktabs = T, escape = FALSE, linesep = c('', '', '\\addlinespace')) %>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>%
  column_spec(1, border_right = T) %>%
  row_spec(0, align = "c")
...
kbl(table_nw_anatolian_site_parameters[, .(Parameter, `Posterior Mean`, `Posterior Median`, `Posterior SD`, `$5\\%$ Quantile`, `$95\\%$ Quantile`, `$\\hat{R}$`, `Effective Sample Size (Bulk)`, `Effective Sample Size (Tail)`)],
    col.names = c("Parameter", "Posterior Mean", "Posterior Median", "Posterior SD", "$5\\%$ Quantile", "$95\\%$ Quantile", "$\\hat{R}$", "Effective Sample Size (Bulk)", "Effective Sample Size (Tail)"),
    caption = "Posterior fit summaries for site-specific model parameters (Northwestern Anatolian cattle)",
    booktabs = T, escape = FALSE, linesep = c('', '', '', '\\addlinespace')) %>%
  kable_styling(latex_options = c("HOLD_position", "scale_down")) %>%
  column_spec(1, border_right = T) %>%
  row_spec(0, align = "c")
...
```{r figure_nw_anatolian_trace, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", out.width = "\\textwidth", fig.cap = "Traceplots of model parameters (Northwestern Anatolian cattle)"}
...
```{r figure_nw_anatolian_prior_posterior, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", out.width = "\\textwidth", fig.cap = "Prior-posterior comparison of Northwestern Anatolian cattle model hyper-parameters"}
