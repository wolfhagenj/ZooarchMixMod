...
modeled_assemblage <- measured_assemblage[, .(ID, Specimen_No)][modeled_assemblage, on = "ID"]

#The final step is to assign the Element_Portion and Immature variables for all specimens.
...
for(i in 1:site_element_key[, .N]) {
  modeled_assemblage[Anatomy %in% site_element_key[i, `Anatomy Label`], Element := site_element_key[i, `Element Label`]]
  #Separate out proximal and distal element portions based on presence of later-fusing measurements (e.g., radius bone is proximal/distal based on presence of Bd measurement)
  if(site_element_key[i, `Element Label`] %in% c("Rad", "Mtc", "Fem", "Mtt")) {
    modeled_assemblage[Anatomy %in% site_element_key[i, `Anatomy Label`], Element := ifelse(`Distal Fusion` %in% "" == F, paste(site_element_key[i, `Element Label`], "dist", sep = "_"), paste(site_element_key[i, `Element Label`], "prox", sep = "_"))]
  }
  if(site_element_key[i, `Element Label`] %in% c("Tib")) {
    modeled_assemblage[Anatomy %in% site_element_key[i, `Anatomy Label`], Element := ifelse(`Proximal Fusion` %in% "" == F, paste(site_element_key[i, `Element Label`], "prox", sep = "_"), paste(site_element_key[i, `Element Label`], "dist", sep = "_"))]
  }
}
