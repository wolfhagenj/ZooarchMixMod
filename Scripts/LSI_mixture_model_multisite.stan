...
model {
  ...
  grand_theta_raw[2] ~ normal(prior_theta_raw_2[1], prior_theta_raw_2[2]);
	...
	element_sigma[1:2] ~ normal(0, 1);
	element_sigma[3] ~ normal(0, 0.1);
	element_sigma[4] ~ normal(0, 0.5);
	element_sigma[5:8] ~ normal(0, 0.25);
	to_vector(z_element) ~ normal(0, 1);
	L_Rho_element ~ lkj_corr_cholesky(2);
	//
	site_sigma[1:2] ~ normal(0, 1);
	site_sigma[3] ~ normal(0, 0.5);
	site_sigma[4:8] ~ normal(0, 0.25);
	to_vector(z_site) ~ normal(0, 1);
	L_Rho_site ~ lkj_corr_cholesky(2);
	//
	interaction_sigma[1:2] ~ normal(0, 0.25);
	interaction_sigma[3:8] ~ normal(0, 0.125); //0.15
  ...
  for(i in 1:N_Measurements) {
    LSI_measurement = log(Measurement[i]) - log(Reference[Measurement_Set[i]]);
